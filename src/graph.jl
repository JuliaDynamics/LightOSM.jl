"""
    graph_from_object(osm_data_object::Union{XMLDocument,Dict};
                      ...
                      compress_dijkstra_states::Bool=false,
...
- `compress_dijkstra_states::Bool=false`: Set true to compress dijkstra parent states (between 15-50% compression), but takes longer to precompute, *NOTE* `getindex` works with compressed arrays.
...
function graph_from_object(osm_data_object::Union{XMLDocument,Dict};
    ...
    precompute_dijkstra_states && add_dijkstra_states!(g, compress_dijkstra_states)
    add_kdtree!(g)
    @info "Created OSMGraph object with kwargs: network_type=$network_type, weight_type=$weight_type, graph_type=$graph_type, precompute_dijkstra_states=$precompute_dijkstra_states, compress_dijkstra_states=$compress_dijkstra_states, largest_connected_component=$largest_connected_component"
    return g
end
...
function graph_from_file(file_path::String;
    ...
    return graph_from_object(obj,
                             ...
                             compress_dijkstra_states=compress_dijkstra_states,
...
"""
function add_dijkstra_states!(g::OSMGraph, compress_dijkstra_states::Bool=false)
    ...
    n > 50000 && @warn "Precomputing and caching all $n Dijkstra States may not be possible due to memory limits, consider compressing Dijkstra States by setting the keyword argument compress_dijkstra_states=true"

    D = compress_dijkstra_states ? TinyIntVec : Vector{U}
    g.dijkstra_states = Vector{D}(undef, n)

    Threads.@threads for src in srcs
        if compress_dijkstra_states
            # compresses array of integers to TinyIntVec - achieves 15%-50% compression
            g.dijkstra_states[src] = pack(dijkstra(g.graph, src, distmx=g.weights, restrictions=g.indexed_restrictions))
        else
