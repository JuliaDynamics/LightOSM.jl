...
function shortest_path(g::OSMGraph,
    ...
    elseif algorithm == :dijkstra
        if save_dijkstra_state && g.dijkstra_states isa Vector
        ...
        elseif save_dijkstra_state && g.dijkstra_states isa TinyIntVec
            # Save compressed state
            parents = pack(dijkstra(g.graph, o_index, distmx=g.weights, restrictions=g.indexed_restrictions))
            g.dijkstra_states[o_index] = parents
        else
            # Do not save state
...
"""
    path_from_parents(parents::Union{Vector,TinyIntVec}, origin::V, destination::V)::Vector where {V <: Integer}

Extracts array of shortest path node ids from a single source dijkstra parent state.

# Arguments
- `parents::Union{Vector,TinyIntVec}`: Array of dijkstra parent states.
- `origin::U`: Origin OpenStreetMap node id.
- `destination::U`: Destination OpenStreetMap node id.
...
- `Vector{U}`: Array of OpenStreetMap node ids making up the shortest path.
"""
function path_from_parents(parents::Union{Vector,TinyIntVec}, origin::V, destination::V)::Vector where {V <: Integer}
...
# Return
- `Vector{U}`: Array of edge weights, distances are in km, time is in hours.
"""
